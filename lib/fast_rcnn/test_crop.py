...
def vis_detections(img_path, im, cls_class_all, dets, thresh):
        crop_img_path="/home/huyangyang/mix/face_crop/"+img_path.split("/")[-1]
        inds = np.where(cls_class_all==2)[0]
        dets=dets[inds, :]
        if len(dets)==0:
            return 
        max_score_ind=np.argmax(np.array(dets[:, -1]))
        max_score = dets[max_score_ind, -1]
        bbox= dets[max_score_ind, :4]
        h, w= im.shape[:2]
        if max_score > thresh:
                bbox[0]=max(bbox[0]-0.1*(bbox[2]-bbox[0]),0)
                bbox[1]=max(bbox[1]-0.1*(bbox[3]-bbox[1]),0)
                bbox[2]=min(bbox[2]+0.1*(bbox[2]-bbox[0]),w-1)
                bbox[3]=min(bbox[3]+0.1*(bbox[3]-bbox[1]),h-1)
...
def test_net(net, img_label, thresh=0.00, vis=False, save_dir=None):
	...
	for i in xrange(len(imgs)):
		...
		if vis:
			vis_detections(imgs[i], im, cls_classes_all, cls_dets_all, cfg.TEST.THRESH)
		if save_dir:
