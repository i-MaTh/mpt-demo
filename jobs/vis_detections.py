	...
	for i in range(min_frame_idx, max_frame_idx):
		...
		for d in sub_dets:
			...
			cv2.rectangle(img, (d[1], d[2]),(d[1] + d[3],d[2] + d[4]),
							  color = colours[d[0]%23], thickness = 3)
...
def show_video(video_dir, dets_path, scale=1.0):
	...
	for i in range(min_frame_idx, max_frame_idx + 1):
		...
		for d in sub_dets:
			trk_id = d[0]
			#if d[2] < 170 or d[1] > 1500 or d[1] < 30 or d[2] > 820: continue
			...
			plt.gca().text(d[1], d[2], 'ID:{}'.format(int(trk_id)), 
								bbox=dict(facecolor=colours[trk_id], alpha=0.5), 
								#fontsize= 0.08*(d[3]), 
								fontsize = 8,
								color='white')
...
def post_processing(img1_dir, img2_dir, output_dir):
	...
	for idx, key in enumerate(img1_paths):
		...
		#img1 = img1[:1000, :, :]
		img1 =cv2.resize(img1, (3840, 1080))
		partition_line = np.zeros((50, 3840, 3))	
		#img2 = img2[:1000, :, :]
		img1 = np.concatenate((img1, partition_line), axis=0)
...
def refine(img_dir, output_dir):
	...
	for idx, key in enumerate(img_paths):
		...
		overlay = img.copy()
		output = img.copy()
		cv2.rectangle(overlay, (position_x + 20, 120), (position_x + 1135, 230),
			              color = (250, 235,215), thickness = -1)

		cv2.putText(overlay, 'Cross Camera Tracking', (position_x + 30, 200),
