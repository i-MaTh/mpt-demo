...
void SmoothL1LossOHEMLayer<Dtype>::LayerSetUp(
  ...
  //when not has normalization params, normalize policy: VALID
  //Divide by the total number of output locations that do not take the
  //ignore_label.  If ignore_label is not set, this behaves like FULL.
  //when policy is BATCH_SIZE, then normalize is pos_num_
  //VALID: labels>=0
  //FULL: labels>=0 U ignore_labels
...
Dtype SmoothL1LossOHEMLayer<Dtype>::get_normalizer(
  ...
  case LossParameter_NormalizationMode_FULL:
    ...
    break;
  case LossParameter_NormalizationMode_POS_NUM:
    normalizer = Dtype(pos_num_);
    break;
