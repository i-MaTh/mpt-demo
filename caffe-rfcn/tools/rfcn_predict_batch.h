...
public:
  RFCN_detector(const string &model_file, const string &trained_file,
               const bool use_GPU, const int device_id,
               YAML::Node node);

  vector<boost::shared_ptr<Blob<float>>> forward(vector<Mat> imgs);
  void nms(vector<struct Bbox> &p, float threshold);
  void get_detection(vector<boost::shared_ptr<Blob<float>>> &outputs, vector<vector<struct Bbox>> &dets, float rfcn_thres, float nms_thres, float resize_height, float resize_width, vector<float>& enlarge_ratio_vec, int image_num);
